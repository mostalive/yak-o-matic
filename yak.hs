...
makeOptions opts = makeOptions' opts emptyOptions
  where
    ...
    makeOptions' (opt:opts)  y@(YakOptions _ _       _)       = makeOptions' opts $ y 
    makeOptions' []          y                                = y 
...
outputCfdData (YakOptions _ (Just gitrepo) (Just filename)) =
